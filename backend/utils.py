...
def apply_filters(rows: List[Dict[str, Any]], q: Optional[str] = None, column_filters: Dict[str, str] = None, settings: Dict[str, Any] = None):
    result = rows
    
    # Global search
    if q and settings and settings["features"]["search"]:
        search = q.lower()
        result = [
            u for u in result 
            if search in u.get('full_name', '').lower() or
               search in u.get('email', '').lower() or
               search in u.get('country', '').lower() or
               search in u.get('status', '').lower() or
               search in str(u.get('age', '')) or
               search in str(u.get('balance_eur', ''))
        ]
